#' @title myApp
#' @description Recruitment Task Appsilon 2020
#' @import shiny shiny.semantic shinycssloaders DT geosphere leaflet dplyr
#' @include calculateDistance.R shipDropdown.R
#' @export
myApp <- function(...){
  
  ## load data
  ships_types_list <<- readRDS(system.file("data/ships_types_list.RDS",
                                           package="AppsilonRecruitmentTask2020"));
  ships_types_names_list <<- readRDS(system.file("data/ships_types_names_list.RDS",
                                                 package="AppsilonRecruitmentTask2020"));
  
  ships_types <<- setNames(ships_types_list$SHIPTYPE, ships_types_list$ship_type);
  addResourcePath("www", system.file("www", package="AppsilonRecruitmentTask2020"));
    ...
    flow_layout(
      ...
      div(class = "ui raised segment",
          div( p(class="ui green ribbon label", "@Appsilon 2020"),
               tags$img(src="www/avatar-appsilon.png",
                        align = "right", height='60',width='60'),
               p(""),
               p("Recruitment task")
          )
      )
  ...
  server <- function(input, output, session) {
    ...
    myData <- reactive({
        readRDS(system.file(paste0("data/ships_data/", shipId(), ".RDS"), 
                            package="AppsilonRecruitmentTask2020"));
    })
